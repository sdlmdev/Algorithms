...
const fs = require('fs');
let [length, ...variants] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n');
let res = 0;

for (let i of new Set(variants)) {
  const variant = i.split(' ');

  if (variant[0] < 0 || variant[1] < 0) continue;
  if (length - 1 - variant[0] == variant[1]) res += 1;
}
