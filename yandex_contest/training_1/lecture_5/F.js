...
// Для каждой модели кондиционера известна его мощность и стоимость.
...
// Третья строка содержит одно целое число m (1 ≤ m ≤ 50 000) —
// количество предложенных моделей кондиционеров.
...
// Гарантируется, что хотя бы один корректный выбор кондиционеров существует,
...
// а в третьем классе – кондиционер третьего типа.
// Суммарная стоимость этих кондиционеров будет составлять 13 рублей (3 + 3 + 7).

const fs = require('fs');
let [numOfClasses, powers, numOfModels, ...models] = fs.readFileSync('input.txt', 'utf8').trim().split('\n').map(i => i.split(' ').map(Number));

numOfModels = numOfModels[0];
numOfClasses = numOfClasses[0];
powers.sort((a, b) => a - b);
models.sort((a, b) => a[1] - b[1] || a[0] - b[0]);

const getMinPrice = (powersArr, modelsArr) => {
  let res = 0;
  let i = 0;

  for (let power of powersArr) {
    while (modelsArr[i][0] < power) {
      i++;
    }
...
fs.writeFileSync('output.txt', getMinPrice(powers, models));