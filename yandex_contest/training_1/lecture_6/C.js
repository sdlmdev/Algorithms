...
// Так как он был достаточно способным мальчиком и усердно учился,
// то на многих из этих олимпиад он получал дипломы.
...
// Для того чтобы эта конструкция выглядела более красиво, Петя хочет,
...
rl.on('line', line => {
  const [w, h, n] = line.split(' ').map(Number);
  ...
  const checkSize = (size, params) => {
    ...
    return Math.floor(size / w) * Math.floor(size / h) >= n;
  };
  ...
  console.log(binarySearch(0, n * Math.max(w, h), checkSize, [w, h, n]));
});