...
// первые два из которых обозначают время открытия кассы в часах и минутах (часы — целое число от 0 до 23,
...
// а время закрытия — что в соответствующую минуту касса уже не работает. Например, касса,
...
// Таким образом, все три кассы одновременно работают с 22 до 23 часов и с часу до двух часов,
// то есть 120 минут.
...
const findOpeningHours = (N, timeArr) => {
  ...
  for (let time of timeArr) {
    const open = time[0] * 60 + time[1];
    const close = time[2] * 60 + time[3];
