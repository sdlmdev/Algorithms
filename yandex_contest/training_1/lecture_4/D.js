...
const fs = require('fs');
const [keysCount, keysDurability, pressesCount, keystrokeSequence] = fs.readFileSync(
  'input.txt', 'utf-8').trim().split('\n').map(line => line.split(' ').map(Number));
const dict = {};

keysDurability.unshift(0);

const fillDictWithKeys = (dictionary, keystrokes) => {
  for (let i of keystrokes) {
    if (i in dictionary) {
      dictionary[i]++;
    } else {
      dictionary[i] = 1;
...
fillDictWithKeys(dict, keystrokeSequence);

const checkKeyStatus = (dictWithKeys, dataDurability) => {
  const res = [];

  for (let key in dictWithKeys) {
    if (dictWithKeys[key] > dataDurability[key]) {
      res.push('YES');
    } else {
      res.push('NO');
    }
...
fs.writeFileSync('output.txt', checkKeyStatus(dict, keysDurability));