...
// идти до ближайшего магазина.
// Для разработки плана развития общественного транспорта на Новом проспекте мэр города
// попросил вас выяснить, какое же наибольшее расстояние приходится преодолевать жителям
// Нового проспекта, чтобы дойти от своего дома до ближайшего магазина.
...
// то между ними расстояние 1, если между двумя домами есть еще один дом,
...
const findMaxPath = (dataArr) => {
  const distanceLeft = Array(dataArr.length);
  let shop = -Infinity;
  let res = 0;

  for (let i = 0; i < dataArr.length; i++) {
    if (dataArr[i] === 2) shop = i;
    if (dataArr[i] === 1) distanceLeft[i] = i - shop;
  }
  ...
  for (let i = dataArr.length - 1; i >= 0; i--) {
    if (dataArr[i] === 2) shop = i;
    if (dataArr[i] === 1) res = Math.max(res, Math.min(shop - i, distanceLeft[i]));
  }
...
fs.writeFileSync('output.txt', findMaxPath(input).toString());
