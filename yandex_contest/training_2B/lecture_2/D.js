...
// если центр плиты приходится на середину какого-нибудь блока, то и слева,
...
// и им не хватает стульев в аудиториях.
...
// Положение ножки определяется расстоянием от левого края плиты до левого края ножки
// (ножка — это куб размером 1×1×1). Ножки перечислены слева направо
...
const findPoints = (L, blocks) => {
  ...
  if (L % 2 && blocks[midIndex] === mid) {
    return blocks[midIndex];
  }

  return `${blocks[midIndex - 1]} ${blocks[midIndex]}`;
};

const fs = require('fs');
const [[l, k], [...points]] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n').map(
  el => el.split(' ').map(Number)
);

fs.writeFileSync('output.txt', findPoints(l, points).toString());