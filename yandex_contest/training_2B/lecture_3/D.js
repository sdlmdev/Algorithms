...
readline.on('line', line => {
  if (res === null) {
    res = new Set(Array.from({ length: Number(line) }, (_, i) => i + 1));
  } else if (line === 'HELP') {
    readline.close();
    console.log([...res].sort((a, b) => a - b).join(' '));
  } else if (line === 'YES') {
    res = new Set([...res].filter(num => curNums.has(num)));
  } else if (line === 'NO') {
    curNums.forEach(num => res.delete(num));
  } else {
    curNums = new Set(line.split(' ').map(Number));
  }
});