...
// — код завершения задачи, во второй — целое число i (0 ≤ i ≤ 7) — вердикт интерактора, в третьей
// — целое число c (0 ≤ c ≤ 7) — вердикт чекера.
...
const fs = require('fs');
const [num, i, c] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n').map(Number);

fs.writeFileSync('output.txt', getResult(num, i, c).toString());
