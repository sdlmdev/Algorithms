...
// По длинам оставшихся на столе N верёвочек определите, какую наименьшую длину может иметь верёвочка,
// взятая Машей.

// Формат ввода
// Первая строка входных данных содержит одно целое число N — количество верёвочек,
// оставшихся на столе (2 ≤ N ≤ 1000).
...
const findMinSegment = (segmentsArr) => {
  const sum = segmentsArr.reduce((acc, el) => acc + el, 0);
  const max = Math.max(...segmentsArr);
  const res = max * 2 - sum;
...
const fs = require('fs');
const [[n], [...segments]] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n').map(
  el => el.trim().split(' ').map(Number)
);

fs.writeFileSync('output.txt', findMinSegment(segments).toString());
