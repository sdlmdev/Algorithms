...
// Формат ввода
// Сначала вводятся натуральное число M — количество секторов на жестком диске (1 ≤ M ≤ 10**9)
...
const countWorkingSystems = (nums) => {
  const segments = new Map();

  nums.forEach((el, i) => {
    let toDelete = [];

    for (let [key, value] of segments.entries()) {
      if (value[1] >= el[0] && value[0] <= el[1]) {
        toDelete.push(key);
      }
    }
...
const fs = require('fs');
const [[m], [n], ...nums] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n').map(line => 
  line.split(' ').map(Number)
);

console.log(countWorkingSystems(nums));