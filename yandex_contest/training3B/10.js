...
// почему бы не продолжить выписывать всеми возможными способами это слово без какой-то части
// с начала и какой-то части с конца?
...
// Формат вывода
// Для каждой буквы на листочке Лёши, выведите её,
// а затем через двоеточие и пробел сколько раз она встретилась в выписанных Лёшей словах
// (см. формат вывода в примерах).
...
readline.on('line', word => {
  ...
  for (let i = 0; i < word.length; i++) {
    const cnt = (word.length - i) * (i + 1);
    
    if (letterCnt[word[i]]) {
      letterCnt[word[i]] += cnt;
    } else {
      letterCnt[word[i]] = cnt;
    }
  }

  let letters = Object.keys(letterCnt).sort();

  for (let letter of letters) {
    console.log(`${letter}: ${letterCnt[letter]}`);
  }
});