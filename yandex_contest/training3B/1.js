...
// Он содержит строчные и прописные латинские буквы, цифры,
...
const fs = require('fs');
const [...input] = fs.readFileSync('input.txt', 'utf-8').trim().split(' ').map(
  el => el.split('')).flat().filter(el => el !== '\n' && el !== ' ');

fs.writeFileSync('output.txt', getHistogram(input));