...
// за которого отдают голоса выборщики этого штата,
...
const getSum = (dataArr) => {
  ...
  for (let el of dataArr) {
    let [person, votes] = el.split(' ');
    votes = Number(votes);

    if (people.has(person)) {
      people.set(person, people.get(person) + votes);
    } else {
      people.set(person, votes);
    }
  }
...
const fs = require('fs');
const items = fs.readFileSync('input.txt', 'utf-8').trim().split('\n');
