...
// оно потеряет свою актуальность. Протокол взаимодействия клиента
// (компьютера, запрашивающего точное время) и сервера
...
// Теперь клиент, из предположения, что сетевые задержки при передаче сообщений от клиента серверу
// и от сервера клиенту одинаковы, может определить и установить себе точное время,
// используя известные значения A, B, C.

// Вам предстоит реализовать алгоритм,
// с точностью до секунды определяющий точное время для установки на клиенте по известным A, B и C.
...
// Все временные метки представлены в формате «hh:mm:ss», где «hh» – это часы, «mm» – минуты,
// «ss» – секунды. Часы, минуты и секунды записываются ровно двумя цифрами каждое
...
const getTime = (A, B, C) => {
  const newA = getSeconds(A);
  const newB = getSeconds(B);
  let newC = getSeconds(C);

  if (newC < newA) {
    newC += 24 * 60 * 60;
  }

  const dif = Math.floor((newC - newA) / 2) + (newC - newA) % 2;
  let newTime = newB + dif;

  if (newTime >= 24 * 60 * 60) {
    newTime -= 24 * 60 * 60;
  }

  const h = Math.floor(newTime / 3600).toString().padStart(2, '0');
  const m = Math.floor((newTime % 3600) / 60).toString().padStart(2, '0');
  const s = Math.round(newTime % 60).toString().padStart(2, '0');
