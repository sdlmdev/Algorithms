...
const abc = Array.from({ length: 3 }, () => [Infinity, Infinity, Infinity]);
const dp = [0, 0, 0];
let n = 0;
let lineCnt = 0;
...
readline.on('line', line => {
  if (lineCnt === 0) {
    ...
    const abcInd = +abc.length;
    const dpInd = +dp.length;
  
    abc[abcInd] = line.split(' ').map(Number);
    dp[dpInd] = Math.min(
      dp[dpInd - 1] + abc[abcInd][0],
      dp[dpInd - 2] + abc[abcInd - 1][1],
      dp[dpInd - 3] + abc[abcInd - 2][2],
    );

    lineCnt += 1;

    if (lineCnt === n + 1) {
      console.log(dp[dpInd]);
      readline.close();
    }
  }
});