...
// на контрольной будут сидеть ровно два ученика. Ученики знают, что варианты будут раздаваться
// строго по порядку: правый относительно учителя ученик первой парты получит вариант 1,
// левый — вариант 2, правый ученик второй парты получит вариант 3 (если число вариантов больше двух)
// и т.д. Так как K может быть меньше чем число учеников N, то после варианта K снова выдаётся вариант 1.
...
// То есть между ними должно оказаться как можно меньше парт, а при наличии двух таких мест с равным
// расстоянием от Пети Вася сядет позади Пети, а не перед ним. Напишите программу, которая подскажет Васе,
...
const getSeatPosition = (place) => {
  let row = Math.floor(place / 2);
  let seat = 2;
  if (place % 2) {
    row += 1;
    seat = 1;
...
rl.on('close', () => {
  const [totalStudents, totalVariants, petyaRow, petyaSeat] = input;
  const petyaPlace = petyaRow * 2 - (petyaSeat === 1 ? 1 : 0);
  const frontPlace = petyaPlace - totalVariants;
  const backPlace = petyaPlace + totalVariants;

  if (totalVariants % 2) {
    if (frontPlace > 0) {
      console.log(getSeatPosition(frontPlace).join(' '));
    } else if (backPlace <= totalStudents) {
      console.log(getSeatPosition(backPlace).join(' '));
    } else {
      console.log(-1);
    }
  } else {
    if (backPlace <= totalStudents) {
      console.log(getSeatPosition(backPlace).join(' '));
    } else if (frontPlace > 0) {
      console.log(getSeatPosition(frontPlace).join(' '));
    } else {
      console.log(-1);
    }
