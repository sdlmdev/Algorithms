...
// решили построить перекрытие.
// Перекрытие будет построено по клеевой технологии и состоять из склеенных друг с другом блоков.
// Блок представляет собой легкий прямоугольный параллелепипед. Два блока можно склеить,
...
// При этом один из углов прямоугольника находится в начале системы координат,
// а другой имеет координаты (W, L). Стены комплекса параллельны осям координат.
...
// Торопясь ввести комплекс в эксплуатацию,
// НГУ решил построить перекрытие из минимально возможного числа блоков.
...
// N — количество возможных блоков (1 ≤ N ≤ 105) и размеры комплекса W и L (1 ≤ W, L ≤ 104).
// Каждая из последующих N строк описывает место монтажа одного блока,
// определяемое координатами противоположных углов: (x1, y1, z1) и (x2, y2, z2),
// при этом 0 ≤ x1 < x2 ≤ W, 0 ≤ y1 < y2 ≤ L, 0 ≤ z1 < z2 ≤ 109.
...
// Формат вывода
// Первая строка выходного файла должна содержать либо слово «YES», если перекрытие возможно построить,
// иначе — слово «NO».
// В первом случае вторая строка выходного файла должна содержать минимальное число блоков,
// образующих перекрытие, а последующие строки — номера этих блоков, в соответствии с порядком,
// в котором они перечислены во входном файле.
...
let lineCount = 0;
let n, m, l, totalSquare, events;

rl.on('line', (line) => {
  if (lineCount === 0) {
    [n, m, l] = line.split(' ').map(Number);
    totalSquare = m * l;
    events = new Array(n * 2);
  } else {
    const [x1, y1, z1, x2, y2, z2] = line.split(' ').map(Number);
    const area = Math.abs(x2 - x1) * Math.abs(y2 - y1);

    events[(lineCount - 1) * 2] = [z1, 1, area, lineCount];
    events[(lineCount - 1) * 2 + 1] = [z2, -1, area, lineCount];
  }
...
rl.on('close', () => {
  ...
  for (let event of events) {
    if (event[1] === 1) {
      nowUsed += 1;
      curArea += event[2];
      if (curArea === totalSquare) {
        minUsed = Math.min(minUsed, nowUsed);
      }
    } else {
      nowUsed -= 1;
      curArea -= event[2];
  ...
  if (minUsed === n + 1) {
    ...
    for (let event of events) {
      if (event[1] === 1) {
        ...
        if (curArea === totalSquare && nowUsed === minUsed) {
