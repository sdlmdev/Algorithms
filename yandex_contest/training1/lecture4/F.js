...
// Пример 3
// Ввод	                          Вывод
// TKSNUU FKXYPUGQ 855146         TKSNUU:
// TKSNUU FKXYPUGQ 930060         FKXYPUGQ 49769497
...
const countOrders = (orders) => {
  ...
  for (let order = 1; order < orders.length; order++) {
    let [name, product, quantity] = orders[order].split(' ');
    quantity = +quantity;
...
fs.writeFileSync('output.txt', countOrders(input));