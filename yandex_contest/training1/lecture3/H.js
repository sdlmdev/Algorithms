...
// Тем же вопросом задались разработчики новой игры.
// В их версии смысл игры прямо противоположен Angry Birds:
// зеленая свинка стреляет по злым птицам из лазерного ружья
// (завязка явно не хуже исходной игры).

// Птицы в игре представляются точками на плоскости.
// Выстрел сбивает только ближайшую птицу находящуюся на линии огня.
// При этом сбитая птица падая сбивает всех птиц, находящихся ровно под ней.
// Две птицы не могут находиться в одной точке.
...
const fs = require('fs');
const [length, ...coordinates] = fs.readFileSync('input.txt', 'utf-8').trim().split('\n');

const res = new Set();

for (let i of coordinates) {
  const elem = i.split(' ')[0];

  res.add(elem);
}

fs.writeFileSync('output.txt', res.size.toString());