...
// Максим действует следующим образом: он включает на тюнере ноты с разными частотами
...
// Формат вывода
// В выходной файл необходимо вывести через пробел два вещественных числа — наименьшее и
// наибольшее возможное значение частоты звучания треугольника, изготовленного Максимом.
...
for (let i = 2; i < input.length; i++) {
  ...
  if (status === 'closer') {
    if (num > prev) {
      left = Math.max(left, (num + prev) / 2);
    } else {
      right = Math.min(right, (num + prev) / 2);
    }
  } else {
    if (num < prev) {
      left = Math.max(left, (num + prev) / 2);
...
fs.writeFileSync('output.txt', left + ' ' + right);