...
// Каждую лепешку, которую метнули участники «Веселого коровяка»,
...
// количество участников чемпионата по метанию лепешек (3 <= n <= 10 ** 5).
...
const [arrLength, arr] = input.split('\n');
const dataArr = arr.split(' ').map(Number);
let res = 1;
let vasilyResult = 0;
let maxResult = dataArr[0];

const checkParticipant = (num) => {
...
for (let i = 1; i < arrLength - 1; i++) {
  if (dataArr[i - 1] > maxResult) {
    maxResult = dataArr[i - 1];
    vasilyResult = 0;
  }

  if (checkParticipant(dataArr[i])) {
    if ((dataArr[i] > vasilyResult) && (dataArr[i + 1] < dataArr[i])) {
      vasilyResult = dataArr[i];
    }
  }
}

if (vasilyResult === 0) {
  return fs.writeFileSync('output.txt', '0');
}

for (let i = 0; i < arrLength; i++) {
  if (dataArr[i] > vasilyResult) {
    res += 1;
  }
}
